...
export default function Navigation({ showAuthLinks = false, backLink, banner }: NavigationProps) {
  const { data: session } = useSession()
  
  // Auto-detect demo mode if no banner prop provided
  const isDemoMode = session?.user?.email === 'demo@careshare.app'
  const shouldShowBanner = banner || isDemoMode
            ...
            <div className={styles.bannerMessage}>
              {banner?.message || (
                <>
                  ...
                  <span>You're exploring with sample data. Feel free to experiment!</span>
                </>
              )}
            </div>
